...
-   Your storage capacity on the CIBERSORTx web portal has a limit of 1 GB (please note that not just uploaded files take disk space but also job results). By experience, if your Seurat object has more than 26,000 cells by 20,000 features (or any combination that would produce a matrix length over 520,000,000) your Reference Matrix will be bigger than 1 GB (you can easily check that in your Seurat object with length(seurat_object$RNA@counts)). This is the reason for the various subset and downsample arguments the function has to offer. You can use them to reduce the size of your Seurat object. I also included a failsafe option to stop the function if, before starting to extract the RNA counts, it detects that the Reference Matrix will be over 1 GB by checking the product of the number of cells by the number of features. This prevents you from waiting a significant amount of time for nothing.
